...
# change the order here to define A1Tokenizer first
class A1Tokenizer:
    ...
    def __call__(self, texts, truncation=False, padding=False, return_tensors=None):
        """Tokenize the given texts and return a BatchEncoding containing the integer-encoded tokens.
           ...
           Returns:
             A BatchEncoding where the field `input_ids` stores the integer-encoded texts and !NEW! `attention_mask` stores the corresponding attention mask.
        """
...
def build_tokenizer(train_file, tokenize_fun=lowercase_tokenizer, max_voc_size=None, model_max_length=None,
    ...
    if max_voc_size is not None:
        ...
        if num_words_to_keep < 0:  
            print(f"Warning: max_voc_size ({max_voc_size}) is smaller than number of special tokens ({len(special_tokens)}).")
            num_words_to_keep = 0
    ...
    if num_words_to_keep != -1:
        ...
        print(f"Truncating vocabulary to {num_words_to_keep} most common words + {len(special_tokens)} special tokens.")
    else:
        # keep all unique tokens
        most_common_words = list(counter.keys())
        print(f"Keeping all {len(most_common_words)} unique words + {len(special_tokens)} special tokens.")
...
if __name__ == "__main__":
    ...
    MAX_VOCAB = 15 # 4 special + 11 words
    MODEL_MAX_LEN = 10
    ...
    # 1. Check size
    print(f"[Check 1.1] Vocab size <= {MAX_VOCAB}?")
    assert len(tokenizer) <= MAX_VOCAB
    print(f"PASS. Size is {len(tokenizer)}.")
    
    # 2. Check special symbols
    print("\n[Check 1.2] Special symbols exist and are correct?")
    ...
    print("\n[Check 1.3] Frequent/Rare words included/excluded?")
    ...
    print("\n[Check 1.4] Round-trip mapping?")
    test_word_in = 'the'
    test_id_in = tokenizer.str_to_int[test_word_in]
    assert tokenizer.int_to_str[test_id_in] == test_word_in
    print(f"PASS. In-vocab: '{test_word_in}' -> {test_id_in} -> '{tokenizer.int_to_str[test_id_in]}'")
    ...
    print(f"PASS. Out-of-vocab: '{test_word_out}' -> {test_id_out} -> '{tokenizer.int_to_str[test_id_out]}'")

    print("\n--- Part 1 Checks Passed ---")

    print("\n\n--- Part 2: Tokenizer Functionality ---")
    ...
    # Check truncation (using MODEL_MAX_LEN = 10)
    long_text = "the the the the the the the the the the the the" # 12 tokens + BOS/EOS = 14
    trunc_encoding = tokenizer(long_text, return_tensors='pt', truncation=True)
    assert trunc_encoding['input_ids'].shape[1] == MODEL_MAX_LEN
    assert trunc_encoding['input_ids'][0, -1] == tokenizer.eos_token_id # Ends with EOS
    print("PASS. Truncation to model_max_length works.")
    
    # Check save and load
    print("\n[Check 2.2] Save and Load:")
    ...
    dataset = load_dataset('text', data_files={'train': DUMMY_TRAIN_FILE, 'val': DUMMY_VAL_FILE})
    print(f"Original dataset sizes: Train={len(dataset['train'])}, Val={len(dataset['val'])}")
    
    # remove empty lines
    print("Filtering empty lines...")
    dataset = dataset.filter(lambda x: x['text'].strip() != '')
    
    print(f"\n[Check 3.1] Filtered dataset sizes:")
    ...
    # optional: subsetting
    print("\n[Check 3.2] Subsetting dataset (optional step):")
    ...
    dl = DataLoader(dataset['train'], batch_size=2, shuffle=False, collate_fn=collate_fn)
    
    print("\n[Check 4.1] Fetching first batch:")
